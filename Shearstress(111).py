...
for fname in filelist:
	...
	plot(data[:12,0],tb,'-r', linewidth=2,label=r'$\tau$ calculated')
	plot(data[:12,0],data[:12,5],'-.g',linewidth=2,label=r'$\sigma_{r}$')
	plot(data[:12,0],data[:12,6],'--b',linewidth=2,label=r'$\sigma_{z}$')
	plot(data[1:,10],t,'oy',linewidth=2,label=r'$\tau$ from experiment')
	xlabel(r'$P_s$ (GPa)', fontsize='14')
	ylabel('Stress (GPa)',fontsize='14')
